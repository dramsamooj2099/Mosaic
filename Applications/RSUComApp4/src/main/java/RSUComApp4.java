 ...
 */
public class RSUComApp4 extends AbstractApplication<RoadSideUnitOperatingSystem> 
...

{
    ...
    private void sendBroadcast(InterVehicleMsg message) {
        ...
        if((routing.getSource().getSourceName().equals("rsu_1")) && (message.getStringMsg() == "Road CLOSED") && (getOs().getSimulationTime() > 240 * TIME.SECOND) && (getOs().getSimulationTime() < 260 * TIME.SECOND))
        {
            message = new InterVehicleMsg(routing, (int) (mInt.intValue() & 0xFF), seqNum++, getOs().getPosition(), "Road OPEN"); 
        }
        else
        {

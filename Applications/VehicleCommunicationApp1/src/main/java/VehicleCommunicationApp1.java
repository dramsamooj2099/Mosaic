...
public class VehicleCommunicationApp1 extends AbstractApplication<VehicleOperatingSystem> 
                              implements CommunicationApplication 
{
     /**
     * Interval at which messages are sent (every 100 milliseconds).
     */
    ...
    private String msg = "Road OPEN";
    private BigInteger mInt;
    static int seqNum = 0;

    @Override
    public void onStartup() {
        getLog().infoSimTime(this, "Initialize application VehicleCommunicationApp1");
    ...
    private void sendAdHocBroadcast() {
       ...
       GeoCircle transmissionArea = new GeoCircle(getOperatingSystem().getPosition(), 300);
        ...
        final MessageRouting routing = getOperatingSystem()
                ...
                .topoBroadCast(1);
        final InterVehicleMsg message = new InterVehicleMsg(routing, (int) (mInt.intValue() & 0xFF), seqNum++, getOs().getPosition(), msg);
        getOs().getAdHocModule().sendV2xMessage(message);
        getLog().infoSimTime(this, ",Sending V2X Message ,{}, {}", this.getOs().getId(), message.toString());
    }
